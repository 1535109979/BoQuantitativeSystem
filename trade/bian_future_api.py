...
from BoQuantitativeSystem.trade.do.account import AccountBook
...
from BoQuantitativeSystem.trade.do.rtn_order import RtnOrder
...
from BoQuantitativeSystem.utils.exchange_enum import OffsetFlag, Direction, OrderPriceType
from BoQuantitativeSystem.utils.sys_exception import common_exception
from BoQuantitativeSystem.utils.thread import submit
    ...
    def _check_listen_ping(self):
        ...
        if ts < 60 * 10:
            self.gateway.on_front_disconnected(
                "连续{}分钟没有收到on_ping回调,重启 _start_listen".format(round(ts / 60, 1)))
            self.close()
            self._start_listen()
